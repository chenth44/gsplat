...
__global__ void rasterize_forward_billboards(
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        int batch_size = min(BLOCK_SIZE, range.y - batch_start);
        for (int t = 0; (t < batch_size) && !done; ++t) {
            const float4 transform = transform_batch[t];
            const float3 xy_opac = xy_opacity_batch[t];
            const float opac = xy_opac.z;
            const float2 delta = {xy_opac.x - px, xy_opac.y - py};
            const float2 uv = {transform.x * delta.x + transform.y * delta.y,
                            transform.z * delta.x + transform.w * delta.y};
            const float sigma = 0.5f * (uv.x * uv.x + uv.y * uv.y);
            const float alpha = min(0.999f, opac * __expf(-sigma));
            if (sigma < 0.f || alpha < 1.f / 255.f) {
                continue;
            }
            ...
            float3 c;
            rgb_grid_bilinear(
                uv, grid_size, color_grids + g * grid_size * grid_size * 3, spatial_scale, c
            );

            // printf("uv %.2f %.2f, color %.2f %.2f %.2f\n", uv.x, uv.y, c.x, c.y, c.z);
