...
__global__ void rasterize_backward_billboards_kernel(
    ...
    __shared__ int32_t id_batch[BLOCK_SIZE];
    __shared__ float3 xy_opacity_batch[BLOCK_SIZE];
    __shared__ float4 transform_batch[BLOCK_SIZE];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            const float opac = opacities[g_id];
            xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            transform_batch[tr] = transforms[g_id];
        }
        // wait for other threads to collect the gaussians in batch
        block.sync();
        // process gaussians in the current batch for this pixel
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if(valid){
                transform = transform_batch[t];
                float3 xy_opac = xy_opacity_batch[t];
                opac = xy_opac.z;
                delta = {xy_opac.x - px, xy_opac.y - py};
                uv = {transform.x * delta.x + transform.y * delta.y,
                      transform.z * delta.x + transform.w * delta.y};
                float sigma = 0.5f * (uv.x * uv.x + uv.y * uv.y);
                vis = __expf(-sigma);
                alpha = min(0.99f, opac * vis);
                if (sigma < 0.f || alpha < 1.f / 255.f) {
                    valid = 0;
            ...
            float3 v_rgb_local = {0.f, 0.f, 0.f};
            float2 v_uv_local = {0.f, 0.f};
            float4 v_transform_local = {0.f, 0.f, 0.f, 0.f};
            float2 v_xy_local = {0.f, 0.f};
            float v_opacity_local = 0.f;
            //initialize everything to 0, only set if the lane is valid
            if(valid){
                // compute the rgb contribution from this gaussian using uv coordinates to index into rgb grid
                int32_t g = id_batch[t];
                float3 rgb;
                rgb_grid_bilinear(
                    uv, grid_size, rgb_grids + g * grid_size * grid_size * 3, spatial_scale, rgb
                );
                ...
                const float fac = alpha * T;
                float v_alpha = 0.f;
                v_rgb_local = {fac * v_out.x, fac * v_out.y, fac * v_out.z}; // local adjoint rgb

                rgb_grid_bilinear_vjp(
                    uv, grid_size, rgb_grids + g * grid_size * grid_size * 3, spatial_scale, v_rgb_local, v_uv_local, v_rgb_grid + g * grid_size * grid_size * 3
                );
                ...
                const float v_sigma = -opac * vis * v_alpha;
                v_uv_local.x += v_sigma * uv.x;
                v_uv_local.y += v_sigma * uv.y;

                v_transform_local = {
                    v_uv_local.x * delta.x, v_uv_local.x * delta.y,
                    v_uv_local.y * delta.x, v_uv_local.y * delta.y
                };

                v_xy_local = {transform.x * v_uv_local.x + transform.z * v_uv_local.y,
                               transform.y * v_uv_local.x + transform.w * v_uv_local.y};

                v_opacity_local = vis * v_alpha;
            }
            warpSum2(v_xy_local, warp);
            warpSum4(v_transform_local, warp);
            warpSum(v_opacity_local, warp);
            if (warp.thread_rank() == 0) {
                int32_t g = id_batch[t];

                float* v_transform_ptr = (float*)(v_transform);
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if(valid){
                ...
                v_conic_local = {0.5f * v_sigma * v_beta_inter * delta.x * delta.x, 
                                        v_sigma * v_beta_inter * delta.x * delta.y,
